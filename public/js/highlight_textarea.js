...
(function($) {
	...
	HighlightWithinTextarea.prototype = {
		...
		getCustomRanges: function (input, custom) {
			let ranges = this.getRanges(input, custom.highlight, custom.colorcode);
			if (custom.category) {
			  ranges.forEach(function (range) {
				...
				range.wordname = custom.wordname;
				range.colorcode = custom.colorcode;
			  });
			}
			return ranges;
		  },
		...
		renderMarks: function(boundaries) {
			...
			input = input.replace(/\{\{hwt-mark-start\|(\d+)\}\}/g, function(match, submatch) {
				...
				if (className) {
					return '<mark style=background-color:'+boundaries[+submatch].colorcode+' class="' + className + '">';
				} else {
					return '<mark>';
				}
			});
	...
	/** RUN TO FUNCTION IMPLEMENTATION */
	$(function () {
		$("#spam-check-main-textarea").highlightWithinTextarea({
		  highlight: [wordArray]
		});
	});
})(jQuery);
